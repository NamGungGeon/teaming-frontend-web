...
class PlayerLog extends Component {
  ...
  componentDidMount() {
    ...
    if (log) {
      convertToEN(log.playInfo.characterName)
        .then(response => {
    ...
    return (
      <div
        className={classNames({
          [styles.win]: playInfo.result === "win",
          [styles.lose]: true,
          clickable: true
        })}
        onClick={() => {
          this.props.router.push(getPath(`/match/${log.matchId}`));
        }}
      >
        <h6>
          [{log.isRank ? "공식" : "일반"}/
          {playInfo.partyUserCount
            ? `${playInfo.partyUserCount}인파티`
            : "솔로"}
          /{playInfo.result === "win" ? "승리" : "패배"}] &nbsp;{log.date}
        </h6>
        <Row>
          <Col xs="8" className={styles.left}>
            <img
              src={thumbnailNoText(this.state.nameEN)}
              alt=""
            ...
            <div>
              ...
              <p style={{ fontSize: "0.7rem", margin: 0 }}>
                플레이타임: {parseInt(playInfo.playTime / 60)}분{" "}
                {playInfo.playTime % 60}초<br />
                {playInfo.killCount}킬 {playInfo.deathCount}데스{" "}
                {playInfo.assistCount}어시스트
                <br />
