...
class Lol extends Component {
    ...
    render() {
        return (
            <div>
                <PageTitle title={'팀 매칭 사전준비'} explain={'리그 오브 레전드 팀 매칭을 위한 사전 정보 입력'} centering/>
                <FormWrapper>
                    <div>
                        ...
                        ]} style={{justifyContent: 'left'}} selections={selection=>{this.setState({...this.state, tier: selection})}}/>
                    </div>
                    <div>
                        ...
                        ]} style={{justifyContent: 'left'}} selections={selection=>{this.setState({...this.state, mainPos: selection})}}/>
                    </div>
                    <div>
                        ...
                        <ImageSelect icons={[
                            ...
                            {img: top, id: 'supporter', label: '서포터'},
                            {img: top, id: 'all', label: '상관없음'},
                        ]} style={{justifyContent: 'left'}} selections={selection=>{this.setState({...this.state, subPos: selection})}}/>
                    </div>
                    <div>
                        <PageTitle title={'챔피언'} explain={'플레이할 챔피언을 선택해주세요 (1개 이상)'} noMargin/>
                        <ImageViewGroup icons={[
                            ...this.state.champions.map((select)=>{
                                return {
                                    img: require(`../../res/lol/champions/${select}.png`),
                                }
                            }),
                            {img: plus, label: '', onClick: ()=>{
                        ...
                        ]} style={{justifyContent: 'left'}} width={'64px'}/>
                    </div>
                    <div>
                        <PageTitle title={'금지 챔피언'} explain={'이 챔피언을 선택한 유저와는 매칭되지 않습니다'} noMargin/>
                        <ImageViewGroup icons={[
                    ...
                    <AlignLayout align={'right'}>
                        <Button color={'primary'} variant={"contained"} size={'large'} onClick={this.requestStart}>시작 &gt;</Button>
                    </AlignLayout>
                </FormWrapper>
                <br/>
            </div>
